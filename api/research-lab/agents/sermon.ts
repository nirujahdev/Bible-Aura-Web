// Sermon Preparation & Generation Assistant API
// Helps with sermon outlines and content generation
...
const BIBLE_SYSTEM_PROMPT = `You are a sermon preparation assistant. You MUST ONLY help with Bible-based sermons. Create sermon outlines, illustrations, applications, and supporting verses. All content must be Bible-focused and theologically sound.`;
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const { notebookId, scriptureReference, sermonType = 'expository', sourceIds } = req.body;
    ...
    if (sourcesError) {
      console.error('[Sermon Agent] Sources error:', sourcesError);
    ...
    const response = await fetch(`${GLM_API_BASE_URL}/chat/completions`, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          { role: 'user', content: `Create a ${sermonType} sermon outline${scriptureContext} using these sources:
...
Format as structured JSON.` }
    ...
    if (!response.ok) {
      ...
      console.error('[Sermon Agent] GLM API error:', errorText);
      return res.status(500).json({ error: 'GLM API error', details: errorText });
    }

    const glmData = await response.json();
    const sermon = glmData.choices?.[0]?.message?.content || 'No sermon outline generated';

    // Save to database
    const outputData = {
      notebook_id: notebookId,
      user_id: userId,
      output_type: 'sermon' as const,
      content: {
        scriptureReference,
        sermonType,
        sermon,
    ...
    if (saveError) {
      console.error('[Sermon Agent] Save error:', saveError);
    } else {
      console.log('[Sermon Agent] Output saved successfully:', savedOutput?.id);
    }
    ...
    console.error('[Sermon Agent] Error:', error);
